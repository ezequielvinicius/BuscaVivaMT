...
Sistema web desenvolvido para a consulta de pessoas desaparecidas e o
envio colaborativo de informações, conforme a especificação da DesenvolveMT.
...
-   **📋 Listagem Paginada**: Cards com foto, dados e status destacado
    (mín. 10/página).
-   **🔍 Busca Avançada**: Filtros por nome, sexo, status e faixa etária
    com debounce.
-   **📄 Detalhamento Completo**: Página individual com informações
    completas e status visual.
-   **📝 Envio de Informações**: Formulário com máscaras, geolocalização
    e upload de fotos.
...
1.  **Clone o repositório**
    ...
    ``` bash
    git clone https://github.com/SEU_USUARIO/buscavivaMT.git
    cd buscavivaMT
    ```
...
Card responsivo exibindo foto, nome, idade, local e status com um design
consistente e claro.
...
-   **Geolocalização por GPS** com fallback para inserção manual de
    endereço.
...
-   **Loading states** com skeleton screens para uma melhor experiência
    de usuário.
-   **Error boundaries** para capturar e tratar erros críticos da
    aplicação.
-   **Fallbacks graciosos** que exibem dados vazios em vez de quebrar a
    aplicação.
-   **Timeout configurável** (padrão de 30s) para requisições.
-   **Retry automático** em caso de falhas de rede, melhorando a
    resiliência.
...
-   **Persistência na URL**: os filtros são mantidos ao recarregar a
    página.
-   **Combinação inteligente**: múltiplos filtros podem ser aplicados
    simultaneamente.
-   **Reset granular**: permite remover filtros individualmente.
-   **Feedback visual**: o contador de resultados é atualizado em tempo
    real.
...
-   **Fallback para endereço textual** caso a geolocalização falhe ou
    não seja permitida.
...
-   **Funcionalidade "Drag & drop"** para uma experiência mais
    intuitiva.
...
-   **Code splitting** com `React.lazy()` para carregar componentes sob
    demanda.
-   **Debounce** de 500ms nas buscas para otimizar o desempenho.
-   **Cache inteligente** de requisições com React Query.
-   **Lazy loading** de imagens para um carregamento inicial mais
    rápido.
...
-   **Validação dupla** (client-side e server-side) para garantir a
...
-   **Semântica HTML5** correta para melhor interpretação por
...
Este diretório contém informações detalhadas sobre a arquitetura,
processos e uso do sistema, além de exemplos de integração com a API.
...
-   `npm run lint:fix`: Corrige automaticamente os erros apontados pelo
    ESLint.
...
-   `npm run docker:run`: Executa o container Docker a partir da imagem
    criada.
...
Para habilitar logs detalhados, inicie o projeto com a flag
`VITE_DEBUG`:
...
-   **ESLint + Prettier** configurados para manter um padrão de código
    consistente.
